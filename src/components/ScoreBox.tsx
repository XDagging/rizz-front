...
<div className='w-full min-h-80 p-2'>
    <div className='font-1 rounded-box bg-base-100 p-3'>
      ...
      <p className='text-5xl font-bold'>{Math.floor(Number(props.charmNum + props.executionNum))}</p>
        </div>
      ...
      <p className='mt-2 border-b pb-4'>You <span className='font-semibold'>do not</span> our college and career readiness benchmarks in Charm and Execution. Lock in twin!</p>
     ...
     <div className='py-4 border-b'>
        ...
        <p className='font-semibold text-2xl'>{Math.ceil((Number(props.charmNum + props.executionNum)/100)*6.2)}th Percentile</p>
        <p className='text-sm'>{Math.ceil((Number(props.charmNum + props.executionNum)/100)*6.2)}% of students scored the same as or below you</p>

     </div>
     <div className="mt-2">
        <p className="text-sm">{new Date(props.dateTaken).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
